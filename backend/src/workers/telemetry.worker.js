export default function processTelemetry(telemetryData) {

    const timestamp = new Date(telemetryData.timestamp);
    const latitude = Number(telemetryData.latitude);
    const longitude = Number(telemetryData.longitude);
    const speed = Number(telemetryData.speed || 0);
    const heading = Number(telemetryData.heading || 0);
